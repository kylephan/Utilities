...
Version - 0.4
Date - 08/1/2019
Description - Bake texture sequences.
'''

import maya.cmds as mc
import os 
projectDir = mc.workspace(q=True,rd=True)
cacheDir = projectDir + 'cache/textures'
## get range
...
def LoadShader(*args):
    global shader
    shader = []
    obj = mc.ls(sl=True, sn = True, dag=True)
    shadingGrps = mc.listConnections(obj,type='shadingEngine')
    shader = mc.ls(mc.listConnections(shadingGrps),materials=True)
    for s in shader:      
...
def LoadTexture(*args):
    ...
    for i in [0,1,2]:
        ...
        fileNode = cmds.listConnections('%s.color' % (shaders[i]), type='file')[0]
        textureFile.append(fileNode.encode('ascii', 'ignore'))   
        textureText = mc.textScrollList('textureTextTextField', append = str(fileNode) , edit = True)
...
mc.textField('outputTextField', text = cacheDir)
mc.text( label = '' )
